...
def plot_scenarios(results):
    ...
    for col, res in enumerate(results):
        ...
        ax_dxy.set_title(
            f"{res['label']}\n{m_str}, {res['total_ok']} reps, {res['elapsed']:.0f}s",
            fontsize=9,
            fontweight="bold",
        )
...
def main():
    print("Kir/Fol 2Ra mating barrier test")
    print(
        f"  P(triple-S in F) = {p_2Ra_S_fol:.2f} x {p_3Ra_S_fol:.2f} x "
    ...
    print(f"{'=' * 60}")
    print(
        f"{'scenario':<20} {'m_eff':>10} {'reps':>6} {'time':>8} "
        f"{'FST_inv':>10} {'FST_col':>10} {'ratio':>8}"
    )
    for res in results:
        print(
            f"  {res['label']:<18} {res['m_eff']:>10.2e} "
