...
def plot_comparison(results_by_nanc):
    ...
    col = 0
    for _ne_anc, (neutral, sweep) in sorted(results_by_nanc.items()):
        for res in [neutral, sweep]:
            ...
            ax_dxy.set_title(
                f"{lbl}, {ne_str}\n{res['total_ok']} reps, {res['elapsed']:.0f}s",
                fontsize=9,
                fontweight="bold",
            )
            ...
            inv_w = [
                w
                for w in range(NW)
                if (inv_3Ra[0] < mid[w] < inv_3Ra[1])
                or (inv_3Rb[0] < mid[w] < inv_3Rb[1])
            ]
...
def main():
    ...
    print(
        f"  Sweep: x_sel={x_sel_3Ra},{x_sel_3Rb}, s={s_coef}, f0={f0}, "
