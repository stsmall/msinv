...
def main():
    ...
    sweep_S = Sweep(
        x_sel=x_sel, t_event=t_sweep_S, target_class="S", selection_coefficient=s_coef
    )
    sweep_I = Sweep(
        x_sel=x_sel, t_event=t_sweep_I, target_class="I", selection_coefficient=s_coef
    )
    ...
    for col, label in enumerate(labels):
        ...
        ax_dxy.plot(
            mid, smooth(dxy), "-", color="#FF9800", lw=2, label="$d_{XY}$ (S vs I)"
        )
        ax_dxy.plot(
            mid,
            dxy_th,
        ...
        ax_pi.plot(mid, smooth(pi_S), "-", color="#1976D2", lw=2, label="$\\pi_S$")
        ax_pi.plot(mid, smooth(pi_I), "-", color="#C2185B", lw=2, label="$\\pi_I$")
        ax_pi.plot(
            mid,
            pi_th_class,
            "--",
        ...
        ax_fst.plot(
            mid, smooth(fst), "-", color="#E65100", lw=2, label="$F_{ST}$ (Hudson)"
        )
        ax_fst.plot(
            mid,
            fst_th,
    ...
    fig.tight_layout()
    fig.savefig("figures/empirical_rdl_sweep.pdf", bbox_inches="tight", dpi=150)
    plt.close()
