    ...
    for w in range(len(wins) - 1):
        ...
        for i in range(len(S)):
            for j in range(i + 1, len(S)):
                d += int((haps[S[i], m] != haps[S[j], m]).sum())
                n += 1
        pi_S[w] = d / max(n, 1) / (wins[w + 1] - wins[w])
        d = n = 0
        for i in range(len(I)):
            for j in range(i + 1, len(I)):
                d += int((haps[I[i], m] != haps[I[j], m]).sum())
                n += 1
...
def main():
    ...
    fig, axes = plt.subplots(
        2, len(GAMMAS), figsize=(15, 6.5), sharex=True, sharey="row"
    )
    ...
    for col, gamma in enumerate(GAMMAS):
        ...
        ax_bot.plot(
            mid, smooth(dxy), "-", color="#C62828", lw=2.2, label=r"$d_{XY}$ (S vs I)"
        )
        ax_bot.plot(
            mid,
            dxy_th_curve,
    ...
    fig.suptitle(
        "Gene flux erodes the karyotype barrier", fontsize=12, fontweight="bold", y=1.00
    )
    ...
    OUT = os.path.join(os.path.dirname(__file__), "..", "figures", "gene_flux_demo.pdf")
    ...
    for gamma in GAMMAS:
        ...
        print(
            f"  {gamma:>10.0e} {pi_i:>10.3e} {pi_c:>10.3e} "
            f"{pi_i / pi_c:>10.2f}  "
