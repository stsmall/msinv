...
def build_sim(scale_cfg, seed):
    ...
    inv_3ra = InversionSpec(
        bp_left=int(0.18 * L),
        bp_right=int(0.18 * L) + w,
        p_inv={0: 0.0, 1: P_INV_3RA_F},
        t_inv=T_INV,
        gene_conversion_rate=GAMMA,
        mean_tract_length=MEAN_TRACT_FRAC * w,
...
def branch_stats_per_window(ts, k, fs, fi, nwin):
    ...
    [(float(edges[i]), float(edges[i + 1])) for i in range(nwin)]
...
def run(scale_name, nreps, nwin, out_prefix):
    ...
    keys = ["pi_K", "pi_FS", "pi_FI", "dxy_K_FS", "dxy_K_FI", "dxy_FS_FI", "Fst_K_F"]
    ...
    for rep in range(nreps):
        ...
        if (rep + 1) % max(1, nreps // 10) == 0:
            ...
            print(
                f"  {rep + 1}/{nreps}  elapsed={elapsed:.1f}s "
    ...
    meta = dict(
        scale=scale_name,
        L=L,
        inv_width=cfg["inv_width"],
        n_reps=n_ok,
        mean_rep_s=float(np.mean(wall)),
        NE_K_CURRENT=NE_K_CURRENT,
        NE_F_PRESENT=NE_F_PRESENT,
        NE_F_AT_SPLIT=NE_F_AT_SPLIT,
        NE_ANC_SPLIT=NE_ANC_SPLIT,
        NE_DEEP=NE_DEEP,
        T_SPLIT=T_SPLIT,
        T_INV=T_INV,
        P_INV_3RA_F=P_INV_3RA_F,
        P_INV_3RB_F=P_INV_3RB_F,
        R=R,
        GAMMA=GAMMA,
    )
    np.savez(f"{out_prefix}.npz", mids=mids, **acc, **meta)
    print(f"\nwrote {out_prefix}.npz  (n_ok={n_ok}, mean rep={np.mean(wall):.3f}s)")
...
def plot_panels(mids, acc, cfg, pdf_path):
    ...
    ax = axes[1, 1]
    ratio = np.where(acc["dxy_FS_FI"] > 0, acc["dxy_K_FI"] / acc["dxy_FS_FI"], np.nan)
