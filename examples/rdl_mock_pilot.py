...
SCALES = {'real': dict(
    inv=REAL_2LA, sweep=REAL_SWEEP, demo=REAL_DEMO,
    samples=REAL_SAMPLES, L=REAL_L, r=REAL_R,
), 'tiny': dict(
    inv=TINY_2LA, sweep=TINY_SWEEP, demo=TINY_DEMO,
    samples=TINY_SAMPLES, L=TINY_L, r=TINY_R,
)}
...
def main():
    ...
    print(f"  L = {cfg['L']:,} bp")
    print(f"  inv = [{cfg['inv']['bp_left']:,}, {cfg['inv']['bp_right']:,}) "
          f"age={cfg['inv']['t_inv']:,} gen")
    print(f"  sweep at bp={cfg['sweep']['x_sel']:,} "
