...
def replot_fig1():
    ...
    ax1.plot(
        mid, pi_th, "--", color="gray", lw=1.0, alpha=0.7, label=r"$E[\pi]=\theta$"
    )
    ax1.axvspan(bp_l, bp_r, color="lightgrey", alpha=0.4)
    ax1.set_xlabel("Position (bp)")
    ax1.set_ylabel("Per-bp diversity")
...
def replot_fig4():
    ...
    fst_A_th = np.where(
        (mid >= inv_A[0]) & (mid <= inv_A[1]), 1.0 - Ne / (inv_A[3] + 2 * Ne), 0.0
    )
    fst_B_th = np.where(
        (mid >= inv_B[0]) & (mid <= inv_B[1]), 1.0 - Ne / (inv_B[3] + 2 * Ne), 0.0
    )
    ...
    ax1.set_xlabel("Position (bp)")
    ax1.set_ylabel(r"$d_{XY}$")
    ax1.set_title("A. Between-karyotype divergence")
    ax1.legend(fontsize=9)

    ax2.plot(
        mid, smooth(fst_A), "-", color="#C62828", lw=2, label=r"Inv A axis $F_{ST}$"
    )
    ax2.plot(
        mid, smooth(fst_B), "-", color="#1565C0", lw=2, label=r"Inv B axis $F_{ST}$"
    )
    ax2.plot(
