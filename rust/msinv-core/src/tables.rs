...
impl TableBuilder {
    ...
    /// Sort edges in tskit canonical order:
    /// `(time[parent] ASC, parent, child, left)`. Called before
    /// returning to Python so the bridge can skip `tc.sort()` (which
    /// dominates the handoff at rho ≥ 2000).
    pub fn sort_edges(&mut self) {
        ...
        indices.sort_by(|&a, &b| {
            ...
            // tskit canonical order: parent time ascending (youngest
            // parent first), then parent id, child id, left.
            ta.partial_cmp(&tb).unwrap()
                .then(self.edge_parent[a].cmp(&self.edge_parent[b]))
                .then(self.edge_child[a].cmp(&self.edge_child[b]))
                .then(self.edge_left[a].partial_cmp(&self.edge_left[b]).unwrap())
        });
        // Apply permutation via cloned temps (simple, O(n) extra space).
