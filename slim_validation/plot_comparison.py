...
def plot(scenario, results_path):
    ...
    def shade(ax):
        ...
        if scenario in SCENARIO_XSEL:
            ax.axvline(SCENARIO_XSEL[scenario], color="#D84315",
                       ls=":", alpha=0.7, lw=1.2, label="_x_sel_")

    c_slim_s = "#1565C0"; c_slim_i = "#6A1B9A"; c_slim_dxy = "#AD1457"
    c_msinv_s = "#2E7D32"; c_msinv_i = "#00838F"; c_msinv_dxy = "#EF6C00"
    c_theory = "#212121"

    # pi panel
    ax_pi.plot(mid, smooth(d["slim_pi_s"]), "-", color=c_slim_s, lw=1.8,
               label=r"SLIM $\pi_S$")
    ax_pi.plot(mid, smooth(d["slim_pi_i"]), "-", color=c_slim_i, lw=1.8,
               label=r"SLIM $\pi_I$")
    ax_pi.plot(mid, smooth(d["msinv_pi_s"]), "--", color=c_msinv_s, lw=1.8,
               label=r"msinv $\pi_S$")
    ax_pi.plot(mid, smooth(d["msinv_pi_i"]), "--", color=c_msinv_i, lw=1.8,
               label=r"msinv $\pi_I$")
    ax_pi.axhline(pi_theory, color=c_theory, ls=":", lw=1.2,
                  label=fr"theory $4N_e\mu$={pi_theory:.1e}")
    shade(ax_pi)
    ax_pi.set_ylabel(r"$\pi$ (per bp)")
    ax_pi.set_title(f"A. Within-karyotype diversity "
                    f"(scenario {scenario}, N_reps={int(d['n_reps'])})",
                    loc="left", fontweight="bold", fontsize=10)
    ax_pi.legend(fontsize=8, loc="upper right", ncol=2)

    # dxy panel
    ax_dxy.plot(mid, smooth(d["slim_dxy"]), "-", color=c_slim_dxy, lw=2.0,
                label="SLIM d_XY(S,I)")
    ax_dxy.plot(mid, smooth(d["msinv_dxy"]), "--", color=c_msinv_dxy, lw=2.0,
                label="msinv d_XY(S,I)")
    ax_dxy.axhline(dxy_outside, color=c_theory, ls=":", lw=1.2,
                   label=fr"theory outside $4N_e\mu$={dxy_outside:.1e}")
    ax_dxy.axhline(dxy_inside, color=c_theory, ls="--", lw=1.2,
                   label=fr"theory inside $2\mu(t_{{inv}}+2N_e)$="
                         f"{dxy_inside:.1e}")
    shade(ax_dxy)
    ax_dxy.set_ylabel(r"$d_{XY}$ (per bp)")
    ax_dxy.set_title("B. Between-karyotype divergence",
                     loc="left", fontweight="bold", fontsize=10)
    ax_dxy.legend(fontsize=8, loc="upper right")

    # Fst panel
    ax_fst.plot(mid, smooth(d["slim_fst"]), "-", color=c_slim_dxy, lw=2.0,
                label="SLIM F_ST")
    ax_fst.plot(mid, smooth(d["msinv_fst"]), "--", color=c_msinv_dxy, lw=2.0,
                label="msinv F_ST")
    fst_inside = 1 - pi_theory / dxy_inside
    ax_fst.axhline(fst_inside, color=c_theory, ls="--", lw=1.2,
                   label=fr"theory inside={fst_inside:.2f}")
    ax_fst.axhline(0, color="#555", ls=":", lw=0.8)
    shade(ax_fst)
    ax_fst.set_ylabel(r"Hudson $F_{ST}$")
    ax_fst.set_xlabel("Position (bp)")
    ax_fst.set_title("C. Karyotype differentiation",
                     loc="left", fontweight="bold", fontsize=10)
