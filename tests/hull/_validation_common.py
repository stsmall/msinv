...
def _run_validation(
    runner_module, scenario_name, engine_a, engine_b, bench_log, n_reps=N_REPS
):
    ...
    assert set(keys) == set(b.keys()), (
        f"stat key mismatch: {engine_a}={set(keys)} vs {engine_b}={set(b.keys())}"
    )

    afs_keys = [k for k in keys if k.startswith("afs_")]
    win_keys = [k for k in keys if k.startswith("pi_window_")]
    moment_keys = [
        k for k in keys if not k.startswith("afs_") and not k.startswith("pi_window_")
    ]
    ...
    _print_benchmark_block(
        scenario_name, engine_a, engine_b, a_secs, b_secs, a_peak, b_peak, bench_log
    )
...
def _print_benchmark_block(
    scenario_name, engine_a, engine_b, a_secs, b_secs, a_peak_kb, b_peak_kb, bench_log
):
    ...
    print(
        f"  {engine_a:7s}: per-rep {a_mean * 1000:6.1f} ms ± {a_se * 1000:.1f}, "
        f"total {a_total:5.1f} s, peak RSS {a_rss_mb:6.1f} MB"
    )
    print(
        f"  {engine_b:7s}: per-rep {b_mean * 1000:6.1f} ms ± {b_se * 1000:.1f}, "
        f"total {b_total:5.1f} s, peak RSS {b_rss_mb:6.1f} MB"
    )
    print(
        f"  ratio:   per-rep {engine_a}/{engine_b} = {a_mean / b_mean:.2f}x;  "
    ...
    record = {
        ...
        engine_a: {
            ...
            "per_rep_se": a_se,
            "total_s": a_total,
            "peak_rss_mb": a_rss_mb,
        },
        engine_b: {
            "per_rep_s": b_mean,
            "per_rep_se": b_se,
            "total_s": b_total,
            "peak_rss_mb": b_rss_mb,
        },
    }
