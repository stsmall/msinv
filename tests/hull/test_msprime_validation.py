...
def _msp_ancestry(n, Ne, L, r, seed, pops=None, demo=None):
    ...
    if demo is None:
        ...
        for pop, count in pops.items():
            sample_sets.append(msprime.SampleSet(count, population=pop, ploidy=1))
        return msprime.sim_ancestry(
            samples=sample_sets,
            demography=demo,
            sequence_length=L,
...
def test_panmictic_diversity_matches_msprime():
    ...
    for seed in range(NREPS):
        ...
        mts2 = msprime.sim_mutations(
            ts2, rate=mu, random_seed=seed + 3000, discrete_genome=False
        )
        msp_pi.append(float(mts2.diversity()))

    ratio = np.mean(hull_pi) / np.mean(msp_pi)
    assert 0.95 < ratio < 1.05, f"Hull/msprime pi ratio = {ratio:.3f}"
...
def test_two_pop_split_dxy_matches_msprime():
    ...
    for seed in range(NREPS_2P):
        ...
        hull_dxy.append(
            float(mts.divergence([list(range(n_per)), list(range(n_per, 2 * n_per))]))
        )
        ...
        demo_m.add_population(initial_size=Ne)
        demo_m.add_mass_migration(time=t_split, source=1, dest=0, proportion=1.0)
        ts2 = _msp_ancestry(
            None, Ne, L, r, seed + 2000, pops={0: n_per, 1: n_per}, demo=demo_m
        )
        mts2 = msprime.sim_mutations(
            ts2, rate=mu, random_seed=seed + 3000, discrete_genome=False
        )
        msp_dxy.append(
            float(mts2.divergence([list(range(n_per)), list(range(n_per, 2 * n_per))]))
        )
...
def test_inversion_cross_class_dxy_elevated():
    ...
    for seed in range(NREPS_INV):
        ts = HullSimulator(
            ...
            recombination_rate=r,
            inversions=[
                InversionSpec(bp_left=bp_l, bp_right=bp_r, p_inv=0.5, t_inv=200_000)
            ],
            seed=seed,
        ).simulate()
        mts = msprime.sim_mutations(
        ...
        if mask.sum() == 0:
            continue
        dc = sum((G[a, mask] != G[b, mask]).sum() for a in S for b in I) / (
            25 * mask.sum()
        )
        ds = sum((G[a, mask] != G[b, mask]).sum() for a in S for b in S if b > a) / (
            10 * mask.sum()
        )
        dxy_cross.append(dc)
        dxy_same.append(ds)
