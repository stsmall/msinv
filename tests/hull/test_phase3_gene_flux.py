...
def test_gene_flux_simple_split_in_middle():
    ...
    while seg is not None:
        ...
        seg = seg.next
    assert outside_intervals == [(0.0, 40.0), (40.0, 100.0)] or outside_intervals == [
        (0.0, 40.0),
        (60.0, 100.0),
    ]
...
@pytest.mark.skip(
    reason="Flux+recomb fragmentation too slow for per-pair path; needs Rust"
)
...
def test_multi_inv_per_inversion_gamma():
    ...
    def counted(active, lin, tl, tr, inv=None):
        if inv is not None:
            counts_per_inv[inv.inv_id] = counts_per_inv.get(inv.inv_id, 0) + 1
