    ...
    for s in S:
        for i in I:
            ...
            assert mrca_time >= t_inv - 1.0, (
                f"Cross-class TMRCA at pos 50 = {mrca_time:.1f}, expected >= {t_inv}"
            )
...
def test_recomb_with_multi_inv():
    ...
    sim = HullSimulator(
        ...
        inversions=[
            InversionSpec(
                bp_left=15_000.0, bp_right=45_000.0, p_inv=0.5, t_inv=100_000.0
            ),
            InversionSpec(
                bp_left=55_000.0, bp_right=85_000.0, p_inv=0.3, t_inv=150_000.0
            ),
        ],
        seed=42,
    )
