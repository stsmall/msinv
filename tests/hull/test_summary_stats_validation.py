...
def _msp_haploid(n, Ne, L, r, seed, pops=None, demo=None):
    ...
    sample_sets = [
        msprime.SampleSet(cnt, population=p, ploidy=1) for p, cnt in pops.items()
    ]
    return msprime.sim_ancestry(
        samples=sample_sets,
        demography=demo,
...
def _mutate(ts, mu, seed):
    return msprime.sim_mutations(ts, rate=mu, random_seed=seed, discrete_genome=False)
...
def test_panmictic_sfs_shape_matches_msprime():
    ...
    for seed in range(nreps):
        ...
        hull_sfs += mts.allele_frequency_spectrum(polarised=True, span_normalise=False)

        ts2 = _msp_haploid(n, Ne, L, r, seed + 2_000)
        mts2 = _mutate(ts2, mu, seed + 3_000)
        msp_sfs += mts2.allele_frequency_spectrum(polarised=True, span_normalise=False)

    # Compare bins 1..n-1 (drop invariant/fixed).
    d = _afs_l2(hull_sfs[1:-1], msp_sfs[1:-1])
    assert d < 0.05, (
        f"SFS L2 distance {d:.3f} exceeds 0.05 (hull={hull_sfs}, msp={msp_sfs})"
    )
...
def test_two_pop_split_segsites_and_dxy():
    ...
    for seed in range(nreps):
        ...
        ts2 = _msp_haploid(
            None, Ne, L, r, seed + 2_000, pops={0: n_per, 1: n_per}, demo=demo_m
        )
        mts2 = _mutate(ts2, mu, seed + 3_000)
...
def test_panmictic_high_rho_segsites_tight():
    ...
    hull_m = np.mean(hull_s)
    msp_m = np.mean(msp_s)
    ratio = hull_m / msp_m
    # Ratio-of-means SE: sqrt((sd_h/m_h)^2 + (sd_m/m_m)^2) / sqrt(n)
    se = np.sqrt(
        (np.std(hull_s) / hull_m) ** 2 + (np.std(msp_s) / msp_m) ** 2
    ) / np.sqrt(nreps)
