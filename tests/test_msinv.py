...
MsinvSimulator = msinv.MsinvSimulator
ConstantFrequency = msinv.ConstantFrequency
DeterministicTrajectory = msinv.DeterministicTrajectory
StochasticTrajectory = msinv.StochasticTrajectory
GeneFluxModel = msinv.GeneFluxModel

# n=2 functions are now in the same package
msinv_n2 = msinv  # simulate_one_n2, build_initial_tree, etc.
...
def test_tinv_scaling():
    ...
    check("dxy(t=100) > dxy(t=20)",
          results[100.0] > results[20.0],
          f"{results[100.0]:.2f} > {results[20.0]:.2f}")
...
def test_n2_vs_n6_consistency():
    ...
    d_n2_inv = 0.0
    for _ in range(NR):
        _, pos, h0, h1 = msinv_n2.simulate_one_n2(
            10.0, 10.0, 1000, 0.5, 0.01, 0.3, 0.7, 0.3,
